...
export const useKeyboard = (params: KeyboardParams) => {
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'e' && status === GameStatus.PLAYING) {
        // DEBUG: Interaction Check - Show on screen
        if (isMenuOpen) {
          setMessage(`BLOCKED: Menu Open (S:${isShopOpen} R:${isRecyclerOpen} I:${isInventoryOpen} C:${isConstructionOpen} A:${currentParams.isArmadilloOpen})`);
          setTimeout(() => setMessage(null), 3000);
        } else {
          // Show position info if not blocked by menu
          // setMessage(`Pos: ${playerRef.current.x.toFixed(1)}, ${playerRef.current.y.toFixed(1)}`);
        ...
        if (!isMenuOpen) {
          ...
          if (!p.isClimbing && atOverworldRope) {
            ...
            else if (currentParams.armadilloX !== undefined && currentParams.wishingWellBuilt && Math.abs(px - currentParams.armadilloX) < 1.5 && currentParams.setIsArmadilloOpen) {
              currentParams.setIsArmadilloOpen(prev => !prev);
            }
